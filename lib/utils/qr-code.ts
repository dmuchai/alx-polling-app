 ...
 */
export function extractPollIdFromQRUrl(url: string): string | null {
  try {
    const urlObj = new URL(url);
    const pathParts = urlObj.pathname.split("/");

    if (pathParts[1] === "polls" && pathParts[2]) {
      return pathParts[2];
    }
...
export async function generateBrandedQRCode(
  ...
  const baseQRCode = await generatePollQRCode(pollId, {
    ...options,
...
export async function generateQRCodeBundle(
  ...
  const [dataUrl, svg, buffer] = await Promise.all([
    generatePollQRCode(pollId, options),
    generatePollQRCodeSVG(pollId, options),
    generatePollQRCodeBuffer(pollId, options),
  ]);
